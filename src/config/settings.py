"""Application configuration for GCP Architecture Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="gcp-architecture-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/gcp_architecture_agent",
        description="Database connection URL",
    )
    redis_url: str = Field(default="redis://localhost:6379/0", description="Redis URL")

    llm_model: str = Field(default="gemini-2.0-flash", description="LLM model ID")
    ...
    vector_store_type: str = Field(default="vertex_ai_search", description="Vector store type")
    opensearch_url: str = Field(default="https://localhost:9200", description="OpenSearch URL")
    opensearch_index: str = Field(default="gcp_architecture_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="gcp-architecture-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="gcp-architecture-agent", description="A2A agent ID")
